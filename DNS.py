...
if flow_regime=='laminar_poi':
    ...
    Re=500 
    dPdx= 0 
    #Lx,Ly,Lz=(2.0*np.pi/1.2, 2.0, 2.0*np.pi/1.8) 
    kx = 1.2
    Lx,Ly,Lz=((2.0*np.pi)/kx,2.0, 1.0)   
    #nx,ny,nz=(8,128,48) 
    nx,ny,nz=(32,81,2) 
    kappa=0.1 #parameter in g_w(t) 
    #g_w = lambda A: 1-np.exp(-kappa*A) #acceleration 
    g_w = lambda A: np.exp(-kappa*A) #decceleration 
    # omega = 1
    # f_rand = lambda A: np.sin(omega*A)
    restart=0 
...
if restart:
    ...
    if flow_regime=='laminar_poi':
        ...
        #mat_contents = sio.loadmat('u_l.mat')
        #u_l = mat_contents['u_l_2D'].squeeze()   # shape (Ny,)
        # Interpolate G onto Dedalus y-grid
        #y_input_grid = np.linspace(-1, 1, len(u_l)) # change to matlab grid
       # f_interp = interp.interp1d(y_input_grid, u_l,
      #                     kind='cubic', fill_value="extrapolate")
      #  G_interp = f_interp(y)

      # u['g'][0] = y #* np.sin(np.pi*(y+1)*0.5) # change this
      # u['g'][0] = u['g'][0] + 0.001 *G_interp
      # u['g'][1] = 0.001 * v_l_2D
      # print("y.shape =", y.shape)
      # print("u.shape =", u['g'][0].shape)
      # print("G_interp shape:", G_interp.shape)
        from scipy.interpolate import RectBivariateSpline

        # --- Load MATLAB data ---
        mat = sio.loadmat('perturbation.mat')

        u_l_2D = mat['u_l_2D']    # (Ny_mat, Nx_mat)
        v_l_2D = mat['v_l_2D']
        w_l_2D = mat['w_l_2D']
        X_mat  = mat['X']
        Y_mat  = mat['Y']
        ...
        #x_mat = mat['X_save'].flatten()
        #y_mat = mat['Y_save'].flatten() # Matlab Grid (N-2 points)

        # Dedalus physical grids
        x1d = x[:, 0, 0]     # shape (nx,)
        y1d = y[0, :, 0]     # shape (ny,)

        # 2D interpolation
        interp_u = RectBivariateSpline(y_mat, x_mat, u_l_2D)
        interp_v = RectBivariateSpline(y_mat, x_mat, v_l_2D)
        interp_w = RectBivariateSpline(y_mat, x_mat, w_l_2D)
        u_xy = interp_u(y1d, x1d)   # (ny, nx)
        v_xy = interp_v(y1d, x1d)   # (ny, nx)
        w_xy = interp_w(y1d, x1d)

        print("u_xy.shape =", u_xy.shape)
        print("v_xy.shape =", v_xy.shape)
        print("w_xy.shape =", w_xy.shape)
        # Expand to 3D

        u_pert = u_xy[None, :, :]  # (1, ny, nx)
        print("u_pert.shape =", u_pert.shape)
        u_pert = np.transpose(u_pert, (2,1,0))  # -> (nx, ny, 1)
        v_pert = v_xy[None, :, :]
        print("v_pert.shape =", v_pert.shape)
        v_pert = np.transpose(v_pert, (2,1,0))
        w_pert = w_xy[None, :, :]
        print("w_pert.shape =", w_pert.shape)
        w_pert = np.transpose(w_pert, (2,1,0))
        print("u_pert.shape =", u_pert.shape)
        print("v_pert.shape =", v_pert.shape)
        
       # alpha = 1.2
       # phase = np.cos(alpha * x1d)[None, None, :]   # (1,1,nx)
       # u_pert = u_hat[:, None] * phase               # (ny,1,nx)
       # v_pert = v_hat[:, None] * phase
        u['g'][0] = y #* np.sin(np.pi*(y+1)*0.5) # change this
        epsilon = 1e-3
        # u['g'][0] = u['g'][0] + epsilon * u_pert
        u['g'][0] += epsilon * u_pert
        u['g'][1] += epsilon * v_pert
        u['g'][2] += epsilon * w_pert
        print("y.shape =", y.shape)
        print("u.shape =", u['g'][0].shape)
        print("u_pert shape:", u_pert.shape)
        print("v_pert shape:", v_pert.shape)
        ...
        z_index = 0  # since your perturbation has only 1 z-plane

        plt.figure(figsize=(8,5))
        plt.pcolormesh(x[:, :, z_index].T, y[:, :, z_index].T, u_pert[:, :, z_index].T, shading='auto')
        plt.colorbar(label='u-perturbation')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.title('u_pert on Dedalus grid (z=0)')
        plt.show()

        plt.figure(figsize=(8,5))
        plt.pcolormesh(x[:, :, z_index].T, y[:, :, z_index].T, v_pert[:, :, z_index].T, shading='auto')
        plt.colorbar(label='v-perturbation')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.title('v_pert on Dedalus grid (z=0)')
        plt.show()

        plt.figure(figsize=(8,5))
        plt.pcolormesh(x[:, :, z_index].T, y[:, :, z_index].T, w_pert[:, :, z_index].T, shading='auto')
        plt.colorbar(label='w-perturbation')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.title('w_pert on Dedalus grid (z=0)')
        plt.show()

        i = 10  # x-index to check

        # MATLAB original curve evaluated on MATLAB grid
        u_orig_mat = u_l_2D[:, i]

        # MATLAB curve re-interpolated onto Dedalus y-grid
        u_orig_interp = interp_u(y1d, x_mat[i])[:, 0]   # shape (ny,)

        plt.figure(figsize=(6,4))
        plt.plot(y_mat, u_orig_mat, 'k--', label='Original (MATLAB)')        # MATLAB grid
        plt.plot(y1d, u_orig_interp, 'b-', label='Original → Dedalus grid')  # interpolated
        plt.plot(y1d, u_pert[i, :, 0], 'r', label='Dedalus interpolated')    # your final field
        plt.legend()
        plt.xlabel('y')
        plt.ylabel('u')
        plt.title('Check interpolation at x-slice i = {}'.format(i))
        plt.grid(True)
        plt.show()
        ...
        # Gaussian noise
        # u['g'][0] = y + np.random.randn(*u['g'][0].shape) * 1e-7 #*np.sin(np.pi*(y+1)*0.5)
